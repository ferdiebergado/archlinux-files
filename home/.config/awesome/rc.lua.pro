...
naughty.config.defaults.font = "Meslo LG S 10"
...
run_once({"sh -c [[ $(pgrep -x xautolock) -eq 0 ]] && xautolock &", "compton -b --xrender-sync --xrender-sync-fence", string.format("notify-send -i /home/d0np0br3/Downloads/images/awesome-logo.png %q", "$(awesome -v | head -n 1 ; systemd-analyze)") })
...
space3 = markup.font("xos4 Terminus 3", " ")
...
local textclock = wibox.widget.textclock(markup(clockgf, space3 .. chosen_clock_type .. markup.font("Misc Tamsyn 3", " ")))
...
local cpu = lain.widget.cpu({
    settings = function()
        widget:set_markup(space3 .. cpu_now.usage .. "%" .. markup.font("Misc Tamsyn 4", " "))
    end
})
...
-- Temp
local temp_icon = wibox.widget.imagebox(beautiful.widget_cpu)
local temp = lain.widget.temp({
    settings = function()
        local fg_color = beautiful.fg_normal
        if coretemp_now >= 60 then
            fg_color = beautiful.bg_warning
            --widget:set_markup(markup.font(beautiful.font, markup.fg.color(beautiful.fg_normal, " +" .. bat_now.perc .. "%")))
        end
        widget:set_markup(markup.font(beautiful.font, markup.fg.color(fg_color, space3 .. coretemp_now .. "C" .. markup.font("Misc Tamsyn 4", " "))))
    end
})

local temp_widget = wibox.container.background(temp.widget)
temp_widget.bgimage=beautiful.widget_display

-- MEM
local mem_icon = wibox.widget.imagebox(beautiful.widget_mem)
local mem = lain.widget.mem({
    settings = function()
        widget:set_markup(space3 .. mem_now.used .. "MB" .. markup.font("Misc Tamsyn 4", " "))
    end
})
...
local net_widgetdl = lain.widget.net({
    iface = "tun0",
    settings = function()
        widget:set_markup(markup.font("Misc Tamsyn 1", " ") .. net_now.received)
    end
})
local net_widgetul = lain.widget.net({
    iface = "tun0",
    settings = function()
        widget:set_markup(markup.font("Misc Tamsyn 1", "  ") .. net_now.sent)
    end
})
...
mpdwidget = lain.widget.mpd({
    settings = function ()
        if mpd_now.state == "play" then
            ...
            widget:set_markup(markup.font("Misc Tamsyn 3", " ")
                              .. markup.font("Misc Tamsyn 7",
                              mpd_now.artist
                              .. " - " ..
                              mpd_now.title
                              .. markup.font("Misc Tamsyn 2", " ")))
            play_pause_icon = wibox.widget.imagebox(beautiful.mpd_pause)
            mpd_sepl = wibox.widget.imagebox(beautiful.mpd_sepl)
            mpd_sepr = wibox.widget.imagebox(beautiful.mpd_sepr)
        elseif mpd_now.state == "pause" then
            widget:set_markup(markup.font("Misc Tamsyn 4", "") ..
                              markup.font("Misc Tamsyn 7", "MPD PAUSED") ..
                              markup.font("Misc Tamsyn 10", ""))
            play_pause_icon = wibox.widget.imagebox(beautiful.mpd_play)
            mpd_sepl = wibox.widget.imagebox(beautiful.mpd_sepl)
            mpd_sepr = wibox.widget.imagebox(beautiful.mpd_sepr)
        else
...
local bat = lain.widget.bat({
    ...
    settings = function()
        ...
        if bat_now.status ~= "Unknown" then
            ...
            widget:set_markup(markup.font(beautiful.font, markup.fg.color(beautiful.bat_full, " AC ")))
            bat_icon:set_image(beautiful.widget_ac)
        end
    end
})
...
globalkeys = awful.util.table.join(
    ...
    awful.key({ modkey            }, "v", function() awful.util.spawn_with_shell("firefox") end ),
